        ...
        return new Task() {
            ...
            protected Integer call() throws Exception {
                ...
                for (int offset = 0; ; offset += 1000) {
                    String JSONAsString = getStringFromURL("http://dbpedia.org/sparql?default-graph-uri=http%3A%2F%2Fdbpedia.org&query=SELECT+%3Fname+%3Fsubject+%7B%7B%0D%0ASELECT+%3Fname+%3Fsubject%0D%0A%7B%0D%0A%3Fbuilding+rdfs%3Alabel+%3Fname++.%0D%0A++%3Fbuilding+a+dbo%3AArchitecturalStructure+.%0D%0A%3Fbuilding+dbp%3AbuildingType+%3Fsubject%0D%0Afilter+langMatches%28+lang%28%3Fname%29%2C+%22pl%22+%29%0D%0A%7D%0D%0Aorder+by+%3Fbuilding%0D%0A%7D%7D%0D%0Aoffset+" + offset + "" +
                            "%0D%0ALIMIT+1000&format=application%2Fsparql-results%2Bjson&CXML_redir_for_subjs=121&CXML_redir_for_hrefs=&timeout=30000&debug=on");
                    ...
                    for (int i = 0; i < bindings.length(); i++) {
                        ...
                        Building aaa = (Building) session.createCriteria(Building.class).add(Restrictions.eq
                        ...
                        if (typeName.contains("http://")) {
                            typeName = typeName.substring(28);
